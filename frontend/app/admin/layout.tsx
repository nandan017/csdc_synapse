        ...
        <div style={{
          ...
          {!collapsed && (
            <div style={{overflow:'hidden'}}>
              ...
              <div style={{fontFamily:'var(--font-jetbrains)',fontSize:9,color:'#666',letterSpacing:'.08em',whiteSpace:'nowrap'}}>Admin Panel</div>
            </div>
          )}
        </div>
        ...
        <div style={{padding:'12px 8px', borderTop:'1px solid #141414'}}>
          {!collapsed && user && (
            <div style={{
              ...
              <div style={{color:'#666'}}>Club Lead</div>
            </div>
          )}
          <button onClick={handleLogout}
            ...
            <span className="nav-icon" style={{color:'#555'}}>⊗</span>
            {!collapsed && <span style={{color:'#555'}}>Sign out</span>}
          </button>
          <button onClick={() => setCollapsed(p=>!p)}
            className="admin-nav-item"
            style={{width:'100%', background:'transparent', border:'none', cursor:'none', textAlign:'left', marginTop:2}}>
            <span className="nav-icon" style={{color:'#555'}}>{collapsed ? '▷' : '◁'}</span>
            {!collapsed && <span style={{color:'#555'}}>Collapse</span>}
          </button>
        </div>
      </aside>
